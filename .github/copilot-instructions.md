...
- **Architecture notes (discoverable patterns)**:
  ...
  - Kubernetes resources are managed as plain YAML under `kubernetes/` (no Helm here). Deployments reference prebuilt images (e.g., ghcr.io/mastodon/mastodon:v4.5.10 in the web deployment).
...
- **Conventions & patterns to follow when editing**:
  - Images are often pinned to explicit versions (e.g., `ghcr.io/mastodon/mastodon:v4.5.10`). Preserve intentional pinning unless bumping versions — note where the version is declared in `deployment-web.yaml`.
